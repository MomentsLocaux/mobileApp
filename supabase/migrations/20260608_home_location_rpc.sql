-- Lot 4 — client write path for the per-user "nearby" reference point.
-- The mobile client only has lat/lon; these RPCs convert to geography and
-- upsert the caller's own preferences row (auth.uid()), so no PostGIS handling
-- leaks to the client and RLS stays simple.
...
begin
    ...
    -- Ignore obviously invalid / null-island coordinates.
