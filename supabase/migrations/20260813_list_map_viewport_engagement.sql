-- MVP-P2-004 follow-up: expose engagement counts on list_map_viewport (after LIMIT).
-- Must DROP first because OUT columns changed.

DROP FUNCTION IF EXISTS public.list_map_viewport(
...
AS $$
  ...
  SELECT
    ...
    COALESCE((
      SELECT COUNT(*)::integer FROM public.event_interests ei WHERE ei.event_id = l.id
    ), 0) AS interests_count,
    COALESCE((
      SELECT COUNT(*)::integer FROM public.event_checkins ec2 WHERE ec2.event_id = l.id
    ), 0) AS checkins_count
  FROM limited l
