-- SCRUM-120 follow-up — Allow operating_hours / schedule_mode on field corrections.
-- Mobile now submits dates + particular hours (same planner as create), place, taxonomy, price.
-- Legacy keys (title, description, cover_url, external_url) stay allowed so pending proposals
-- and older clients still validate. Do NOT apply without human validation.
...
BEGIN
  ...
  IF NEW.kind = 'field_correction' AND NEW.proposed_fields IS NOT NULL THEN
    ...
    IF NEW.proposed_fields ? 'schedule_mode' THEN
      v_json_type := jsonb_typeof(NEW.proposed_fields->'schedule_mode');
      IF v_json_type IS DISTINCT FROM 'string'
         OR (NEW.proposed_fields->>'schedule_mode') NOT IN ('ponctuel', 'recurrent') THEN
        RAISE EXCEPTION 'EVENT_CORRECTION_SCHEDULE_MODE_INVALID' USING ERRCODE = 'P0001';
      END IF;
    END IF;

    IF NEW.proposed_fields ? 'operating_hours' THEN
      v_json_type := jsonb_typeof(NEW.proposed_fields->'operating_hours');
      IF v_json_type IS DISTINCT FROM 'null'
         AND v_json_type IS DISTINCT FROM 'array'
         AND v_json_type IS DISTINCT FROM 'object' THEN
        RAISE EXCEPTION 'EVENT_CORRECTION_OPERATING_HOURS_INVALID' USING ERRCODE = 'P0001';
      END IF;
    END IF;
  END IF;
...
COMMENT ON COLUMN public.event_correction_proposals.proposed_fields IS
  'Partial event field diff for kind=field_correction. User form: schedule (incl. operating_hours), place, taxonomy, price. Legacy title/description/cover/url still accepted.';
