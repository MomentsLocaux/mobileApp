...
Titre: Alerte « tu es près d’un moment en cours / bientôt » (approche live)

Priorité: P0 — cœur produit push-first

Source: Vision push « Il y a ça à côté de toi maintenant / ce soir » ; écart vs nearby actuel (fan-out à la publication seulement)

Responsable / agent recommandé: Mobile Reliability Engineer, GDPR / Store Compliance Officer, UX/UI Guardian

Type d'action: Mobile + Edge/SQL + permissions store

Fichiers probablement concernés:

- `src/tasks/` (geofence ou significant-location, distinct de `discovery-location` si possible)
- `src/services/push.service.ts` / nouveau `proximity-alert.service.ts`
- Edge ou RPC : events live dans un rayon autour de lat/lng
- `app/settings/notifications.tsx` (opt-in explicite + copy App Store)
- `Info.plist` / `AndroidManifest` (Always / background location si requis)
- Déduplication : table ou prefs `last_proximity_alert_at` / event ids alertés

Description:

Aujourd’hui « Nouveaux moments près de moi » = un event **vient d’être publié** dans le rayon de `home_location` (snapshot). Ce n’est **pas** une alerte quand le téléphone **s’approche** d’un event déjà publié / en cours.

Livrer le cœur vision :

1. Opt-in clair (réglages + permission OS) — distinct du consent Discovery si possible.
2. Détection approche : MVP recommandé = **geofencing** des events « live / bientôt » dans un rayon utile (limites OS ~20 regions iOS) **ou** significant location change + requête nearby live ; pas de tracking continu brut stocké serveur sans minimisation.
3. Insert `notifications` type dédié (ex. `event_nearby_live` — migration enum) ou réutiliser un type avec `data.source = 'proximity_live'`.
4. Respecter plafond journalier, ne pas déranger, thèmes, anti-doublon (pas 10 notifs pour le même event).
5. Copy store : justification localisation « pour vous prévenir des moments près de vous ».

Critères d'acceptation:

- Scénario device : event publié à 200 m, user se rapproche → push (UAT, build natif)
- Sans opt-in / permission → aucune collecte background pour ce flux
- Pas de spam : 1 alerte / event / fenêtre (ex. 24 h)
- `typecheck` + `lint` ; runbook + QA matrix mis à jour
- Revue GDPR / store (copy, désactivation facile)

Risques: Rejet App Store (background location) ; batterie ; confusion avec Discovery capture ; limite de regions geofence

Dépendances: PUSH-P0-001 (anti-spam), prefs nearby existantes

Branche Git recommandée: `feat/push-p0-live-proximity`
...
- Toggle séparé « seulement moments créés par des habitants » (UGC) — possible en P1 après PUSH-P0-003
