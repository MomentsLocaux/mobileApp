...
import {
  POSTER_ANALYSIS_STEPS,
  posterAnalysisEaseProgress,
  posterAnalysisPercent,
  posterAnalysisStepStatus,
  type PosterAnalysisStepId,
} from '@/utils/poster-analysis-progress';
...
export function PosterAnalysisProgress({ stepId, complete = false }: Props) {
  ...
  const stepProgress = complete ? 1 : reduceMotion ? 0 : posterAnalysisEaseProgress(elapsedMs);
  const percent = posterAnalysisPercent(stepId, stepProgress, { complete });
  const activeLabel =
    POSTER_ANALYSIS_STEPS.find((step) => step.id === stepId)?.label ?? 'Analyse en cours';

  const steps = useMemo(
    () =>
      POSTER_ANALYSIS_STEPS.map((step) => ({
        ...step,
        status: posterAnalysisStepStatus(step.id, stepId, { complete }),
      })),
    [complete, stepId],
  );
  ...
  return (
    <View
      ...
      accessibilityLabel={`Analyse de l’affiche, ${percent} pour cent. ${activeLabel}.`}
      accessibilityValue={{ min: 0, max: 100, now: percent }}
    >
      <View accessibilityElementsHidden importantForAccessibility="no-hide-descendants">
        <BrandLogoSpinner size={56} accessibilityLabel="Analyse en cours" />
      </View>
      <Text style={styles.percent}>{percent} %</Text>
      <Text style={styles.caption}>{complete ? 'C’est prêt' : activeLabel}</Text>
