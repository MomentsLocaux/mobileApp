...
import { Heart, Flag, Star, Image as ImageIcon } from 'lucide-react-native';
import { AppBackground, Button, Card } from '@/components/ui';
import { colors, spacing, typography, borderRadius } from '@/constants/theme';
import { useAuth } from '@/hooks';
import { useComments } from '@/hooks/useComments';
import { useEventCheckinStatus } from '@/hooks/useEventCheckinStatus';
...
export default function EventEchoesScreen() {
  ...
  const { hasCheckedIn } = useEventCheckinStatus(event?.id, profile?.id);
  ...
  const organizerUrls = useMemo(() => {
    const urls = [
      event?.cover_url,
      ...((event?.media || []).map((m) => m.url).filter(Boolean) as string[]),
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.headerRow}>
        ...
        <Text style={styles.headerTitle}>Echos de la communauté</Text>
      </View>

      <View style={styles.tabRow}>
        <TouchableOpacity style={[styles.tab, tab === 'reviews' && styles.tabActive]} onPress={() => setTab('reviews')}>
          <Text style={[styles.tabText, tab === 'reviews' && styles.tabTextActive]}>Avis</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  headerRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, paddingHorizontal: spacing.lg, paddingTop: spacing.xl, paddingBottom: spacing.md },
  back: { ...typography.bodySmall, color: colors.brand.secondary, fontWeight: '700' },
  ...
  tab: { paddingHorizontal: spacing.md, paddingVertical: spacing.xs, borderRadius: borderRadius.full, backgroundColor: 'rgba(255,255,255,0.06)' },
  ...
  photo: { width: 110, height: 110, borderRadius: borderRadius.md, backgroundColor: 'rgba(255,255,255,0.08)' },
});
