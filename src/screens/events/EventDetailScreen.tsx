...
export default function EventDetailScreen() {
  ...
  const formatDateRange = (start: string, end?: string | null) => {
    ...
    if (startDate.toDateString() === endDate.toDateString()) return formatDate(start);
    return endDate < startDate ? `${formatDate(end)} - ${formatDate(start)}` : `${formatDate(start)} - ${formatDate(end)}`;
  };
  ...
  const formatTimeRange = (start: string, end?: string | null) => {
    ...
    const startDate = new Date(start);
    const endDate = new Date(end);
    return endDate < startDate ? `${formatTime(end)} - ${formatTime(start)}` : `${formatTime(start)} - ${formatTime(end)}`;
  };
    ...
    <>
      ...
      >
        ...
        <View style={styles.content}>
          ...
          <TouchableOpacity style={styles.cardTouchable} activeOpacity={0.85} onPress={openCalendar}>
            <Card padding="md" style={styles.infoCard}>
              <View style={styles.infoRowNoMargin}>
                ...
                <View style={styles.infoContent}>
                  ...
                  <Text style={styles.infoLabel}>{formatTimeRange(event.starts_at, event.ends_at)}</Text>
                </View>
          ...
          <Card padding="md" style={[styles.infoCard, { marginTop: spacing.md }]}> 
            <View style={styles.infoRowNoMargin}>
              ...
              <TouchableOpacity style={styles.routeButton} onPress={() => setNavSheetVisible(true)}>
                <Text style={styles.routeText}>Voir tout</Text>
              </TouchableOpacity>
            </View>
          </Card>
          ...
          >
            <View style={{ flex: 1, gap: 4 }}>
              ...
              <Text style={styles.xpSubtitle}>
                Participez à cet événement pour gagner <Text style={{ fontWeight: '800' }}>+{XP_REWARD} XP</Text>{' '}
      ...
      <View style={[styles.bottomBar, { paddingBottom: Math.max(insets.bottom, spacing.sm) }]}> 
        <View style={styles.bottomBarContent}>
          <TouchableOpacity style={[styles.bottomBarCta, styles.bottomPrimary]} onPress={handleToggleFavorite}>
            ...
            />
            <Text style={[styles.bottomBarText, { color: colors.brand.text }]}>Favoris</Text>
          </TouchableOpacity>

          <TouchableOpacity style={[styles.bottomBarCta, styles.bottomSecondary]} onPress={handleCheckIn} disabled={hasCheckedIn || isOwner}>
            <Navigation2 size={18} color={colors.brand.text} />
            <Text style={[styles.bottomBarText, { color: colors.brand.text }]}>{hasCheckedIn ? 'Check-in validé' : 'Check-in'}</Text>
          </TouchableOpacity>
        </View>
      </View>
