...
| EventCard (listes + overlay map) | `EventCard`, `EventResultCard`, Home, favoris, bottom sheet, `EventsListScreen` | Même corps `map-preview` : cover 220, titre, ligne date calendrier, lieu ; pas de panneau DÉBUT/FIN, pas de description, pas de preuve sociale (celle-ci reste sur la fiche événement) |
| EventCard social proof | `src/screens/events/EventDetailScreen.tsx` | Liker avatar stack (follows first) + leaf count on the **detail** screen; list cards stay épurées |
